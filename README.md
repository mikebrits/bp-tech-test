...
### Stack
 - **State management**: Redux (with redux hooks) and React Hooks for local state
...
```
While the structure is not important, something to take note of is that the index file only exports `MyComponent`. In doing so, this gives us a "buffer" layer where we might have hooked up redux to the component. This allows us to test the component purely, and not have to import redux providers in our tests. Also, should the way in which we get the data, (ie, not from a redux store, but from a server) change, then it's super easy to move components around, and the only thing that needs to change is the buffer layer. FYI

Disclaimer: Like the generator, there are components I have developed for other private projects that were useful here, specifically the Popover used on the dropdown menu. I didn't want to have all of Bootstrap / antd / material-ui for one component.
